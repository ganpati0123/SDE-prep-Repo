...
export default function Dashboard() {
  ...
  const avgAccuracy = solvedAll.length > 0
    ? Math.round(solvedAll.reduce((sum, a) => sum + (a.accuracy || 0), 0) / solvedAll.length)
    : 0
    ...
    <>
      ...
      <div className="content-area">
        ...
        <div className="phase-banner animate-fadeIn">
          <div className="phase-number" style={{ background: currentPhase?.color }}>
            {schedule.phase}
          </div>
          <div className="flex-1">
            <div style={{ fontSize: '16px', fontWeight: 700 }}>{currentPhase?.name}</div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              Day {todayDay} — {schedule.dsaTopic}
            </div>
          </div>
          <button className="btn btn-primary" onClick={() => navigate(`/day/${todayDay}`)}>
        ...
        <div className="stat-grid">
          ...
          <div className="stat-card animate-fadeIn">
            ...
            <div className="stat-value">{solvedToday.length} / {schedule.questions.length}</div>
            ...
            {currentBlock ? (
              <div>
                ...
                {currentBlock.type === 'study' && (
                  <button className="btn btn-primary w-full" onClick={() => navigate('/focus')}>
                    <Eye size={14} />
                ...
                return (
                  ...
                  >
                    ...
                    {attempt?.status === 'solved' ? (
                      <div className="question-status status-solved">
                        <CheckCircle size={16} />
                      </div>
                    ) : (
              ...
              {weakTopics.slice(0, 5).map(t => (
                <div key={t.id} className="weak-topic-item">
                  <AlertCircle size={16} color={
                    t.severity === 'high' ? 'var(--error)' : t.severity === 'medium' ? 'var(--warning)' : 'var(--text-tertiary)'
                  } />
                  <div className="flex-1">
                    <div className="font-600">{t.topic_name}</div>
                    {t.notes && <div className="text-sm text-tertiary">{t.notes}</div>}
                  </div>
                  <span className={`badge ${t.severity === 'high' ? 'badge-hard' : 'badge-medium'}`}>
                    {t.severity}
                  </span>
                </div>
              ))}
