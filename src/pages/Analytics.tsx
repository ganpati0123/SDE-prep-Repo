      ...
      <div className="content-area">
        ...
        <div className="grid-2">
          <div className="card animate-fadeIn">
            <div className="section-title"><Award size={18} style={{ color: 'var(--amber-bright)' }} /> Problems by Difficulty</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {[
                { label: 'Easy', count: easySolved, color: 'var(--green)', badge: 'badge-easy' },
                { label: 'Medium', count: mediumSolved, color: 'var(--amber)', badge: 'badge-medium' },
                { label: 'Hard', count: hardSolved, color: 'var(--red)', badge: 'badge-hard' },
              ].map(d => (
                <div key={d.label}>
          ...
          <div className="card animate-fadeIn">
            <div className="section-title"><Clock size={18} style={{ color: 'var(--cyan-bright)' }} /> Focus Time by Day</div>
                ...
                {Object.entries(focusByDay).sort((a, b) => parseInt(a[0]) - parseInt(b[0])).map(([day, sec]) => (
                  <div key={day} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div className="progress-bar" style={{ flex: 1 }}>
                      <div className="progress-bar-fill" style={{ width: `${Math.min(100, (sec / 3600) * 100)}%`, background: 'var(--grad-blue-cyan)' }} />
                    </div>
        ...
        <div className="card animate-fadeIn mt-4">
          <div className="section-title"><Target size={18} style={{ color: 'var(--teal-bright)' }} /> Topic-wise Progress</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '12px' }}>
            {sortedTopics.map(([topic, stats]) => (
              <div key={topic} className="card" style={{ background: 'var(--bg-tertiary)', padding: '14px' }}>
                ...
                <div className="progress-bar">
                  <div className="progress-bar-fill" style={{ width: `${stats.total > 0 ? (stats.solved / stats.total) * 100 : 0}%`, background: stats.solved === stats.total && stats.total > 0 ? 'var(--green)' : 'var(--grad-blue-cyan)' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card animate-fadeIn mt-4">
          <div className="section-title"><TrendingUp size={18} style={{ color: 'var(--pink-bright)' }} /> Phase Progress</div>
